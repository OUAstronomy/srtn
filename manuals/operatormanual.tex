...
\title{\Huge \textbf{The University of Oklahoma} \\ \textbf{Nielsen Hall SRT}\\ \huge Operator's Manual \\ \huge Version 0.1}
% Author
...
Multiple directories probably already exist on the computer. Most of these directories will be personal ones and please respect the integrity of other people's work. There exist two simulation5/6 directories which are for public use and can be used simultaneously with the main srtn program. These are for determining observation plans for later times and they can be used to simulate the positions of objects at any time.
...
\begin{itemize}
    ...
    \item The metadata parser is called all\_meta\_parse.py. The all\_meta\_parse script lists columns of data in the order: Date, obsn, az, el, center freq, Tsys, Tant, vlsr, galactic latitude, galactic longitude, source, fstart, fstop, spacing, bw, fbw, nfreq, nsam, npoint, integ, sigma, bsw.
    \item For spectrum analysis:
    \item The spectrum parser is called all\_hispec.py. The all\_hispec script lists: frequency, vel, vel-vlsr, and Tant. 
    \item To run the files, use \code{$\sim$\$ python all\_hispec.py --test}. Follow the input parameters or just simply run \code{python all\_hispec.py }
    \item Once you have your spectra parsed out and written to a new file, you can start to plot them all. 
    \item You can reduce and plot the spectra by running the code \code{python spec\_reduc.py --help}. This code will walk you through the reduction, but to summarize the steps you are flattening the baseline, removing RFI, and then finding the integrated intensity.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Gnuplot}
