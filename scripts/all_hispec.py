...
if backup_files in files: files.remove(backup_files)
...
if answer == 1:
    ...
    for f in files:
        ...
        with open(f,'r') as feel:
	        lines = [line.strip() for line in feel.readlines() if len(line.strip()) != 0]
...
# second section
    ...
    print("Running file: " + f)
    filelength = len(open(f).readlines())
    lines = [line.strip() for line in f.readlines() if len(line.strip()) != 0]

    outname_array = []
    bufferfile="buffer_" + f
